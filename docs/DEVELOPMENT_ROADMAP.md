...
| **P9 — Date-aware historical GDELT** | Replaces the crypto-only DOC query + 90-day guard with **per-window curated queries** (COVID / War / Election / Iran) and an 8-year lookback. Topical breadth: 6-category theme map (war / election / covid / regulation / macro / crypto) so the headline panel and theme river stay populated through non-crypto windows. Bulk-warm script seeds the cache for 32 hot dates (Black Thursday, Russia invasion, election day, BTC $100k, …). | `gdelt_curated.py`, `gdelt_service.py`, `fetch_gdelt_historical.py`, `DATA_AND_APIS.md`, `USER_GUIDE.md`, `DEMO_SCRIPT.md` | backend-only (0 KB frontend) | ✅ |
